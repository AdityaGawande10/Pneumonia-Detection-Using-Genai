...
<head>
    ...
    <style>
        ...
        .delete-button, .done-button, .feedback-button {
            padding: 5px 10px;
            border-radius: 4px;
            cursor: pointer;
            border: none;
        ...
        <table>
            ...
            <?php while ($row = $appointment_result->fetch_assoc()): ?>
                <?php 
                    ...
                    $feedback_query = "SELECT * FROM feedback WHERE appointment_id = '$appointment_id'";
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php elseif ($row['status'] == 'Done' && $has_feedback): ?>
                            <a href="viewfeedback.php?appointment_id=<?php echo $row['id']; ?>">
                                <button class="feedback-button">View Feedback</button>
                            </a>
                        <?php elseif ($row['status'] == 'Done'): ?>
                            <button class="feedback-button" disabled>No Feedback Submitted</button>
                        <?php endif; ?>
